...
- 日期始终使用实际日期
- 搜索功能必须有完整的错误处理
- KaTeX 修复：`math_engine: null` + JS 修复 `$...$` 内 `<em>` → `_`
- 不要用 `document.body.innerHTML = html` 的方式替换 DOM，会销毁事件监听器
- 前端 JS 修改后必须确认大括号闭合，搜索 JS 特别容易漏掉 `}`
- 标签要有区分度：如果每篇文章都有的内容（如 Python 代码），不要作为标签
- 图片统一放 `assets/images/`，引用时用 `{{ site.baseurl }}/assets/images/文件名`
